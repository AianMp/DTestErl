...
auxPing([H|T], NodeList)->
    {peer, H}!{ping,self(),node()},
    receive
	{pong,_,Node} ->
	    %io:format("***CLIENT***     (ping)-Node ~p available!~n",[Node]),
	    auxPing(T,[Node|NodeList])
    after 
	500 ->
	    %io:format("***CLIENT***     (ping)-Node ~p NOT available!~n", [H]),
	    auxPing(T,NodeList)
    end.
...
send([{Num,Task,Node}|T])->
    {peer, Node} ! {self(), node(),{Num ,Task}},
    %io:format("***CLIENT***     (send)-Task has been sent to node: ~p~n",[{Node, Num,Task}]),
    send(T).
...
loopClient(Results) ->
    receive
	{ok, _, Node, Num, Task}->
	    io:format("***CLIENT***     (loopClient)-Has received the result: ~p~n",[{Num,Task,Node}]),
	    loopClient([{Num,Task,Node}|Results]);
	{stop} ->
	    %TODO no llega...
	    io:format("~n***********The client ends!************~n")    
    after 10000 ->
	    controller(Results)
    end.
...
controller(Results)->
    ...
    TaskRun = lists:subtract(TaskNodeList,Results),
    case TaskRun of
	[]->
	    io:format("~n***********The client ends!************~n"),
	    print(Results),
	...
	L  ->
	    ...
	    io:format("***CLIENT***     (controller)-Lost ~p tasks ~p~n", [length(TaskError), TaskError]),
	    case TaskError of
		[] ->
		    loopClient(Results);
		_ ->
